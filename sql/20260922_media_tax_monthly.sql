...
create table if not exists public.moises_media_tax_monthly (
  ...
  invoices_complete boolean not null default false,
  ...
  constraint tax_requires_separate_media check (
    media_excludes_tax or estimated_rate is null
  )
);
...
-- Cada nota pertence à conta de anúncio e à loja selecionada. Nunca soma
-- automaticamente contas de lojas diferentes em uma mesma visão.
create table if not exists public.moises_media_invoices (
  workspace_id uuid not null,
  month date not null,
  account_label text not null check (length(trim(account_label)) between 1 and 100),
  invoice_base numeric(14,2),
  invoice_total numeric(14,2) not null check (invoice_total > 0),
  updated_at timestamptz not null default now(),
  primary key (workspace_id, month, account_label),
  foreign key (workspace_id, month)
    references public.moises_media_tax_monthly(workspace_id, month) on delete cascade,
  constraint invoice_base_valid check (invoice_base is null or (invoice_base >= 0 and invoice_base <= invoice_total))
);

alter table public.moises_media_invoices enable row level security;
revoke all on public.moises_media_invoices from anon;
grant select, insert, update on public.moises_media_invoices to authenticated;

create policy "Members read media invoices"
  on public.moises_media_invoices for select to authenticated
  using (exists (
    select 1 from public.moises_workspace_members m
    where m.workspace_id = moises_media_invoices.workspace_id
      and m.user_id = (select auth.uid())
      and m.role in ('admin', 'viewer')
  ));

create policy "Admins insert media invoices"
  on public.moises_media_invoices for insert to authenticated
  with check (exists (
    select 1 from public.moises_workspace_members m
    where m.workspace_id = moises_media_invoices.workspace_id
      and m.user_id = (select auth.uid()) and m.role = 'admin'
...
create policy "Admins update media invoices"
  on public.moises_media_invoices for update to authenticated
  using (exists (
    select 1 from public.moises_workspace_members m
    where m.workspace_id = moises_media_invoices.workspace_id
      and m.user_id = (select auth.uid()) and m.role = 'admin'
